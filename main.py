...
def generate_house_data():
    ...
    df.to_excel('house.xls')

def load_house_data():
    return pd.read_excel('house.xls')
...
generateb = right.button('generate house.xls')
if generateb:
    right.write('generating "house.xls" ...')
    generate_house_data()
    right.write(' ... done')

loadb = right.button('load house.xls')
if loadb:
    right.write('loading "house.xls ..."')
    df = pd.read_excel('house.xls', index_col=0)
    right.write('... done')
    right.dataframe(df)
    fig, ax = plt.subplots()
